...
from vlm_response_analyzer import VLMResponseAnalyzer
...
def evaluate(
  ...
  for i in range(len(images)):
    if prompt_data_idx >= len(prompt_datas):
      break

    image = images[i]
    prompt_data = prompt_datas[prompt_data_idx]

    if isinstance(image, str):
      img_data_idx = int(image.split('/')[-2].split('_')[0])
      if img_data_idx < prompt_data['data_idx']:
        continue

      image = Image.open(image)


    print(f"Processing data {prompt_data['data_idx']}...")
    ...
    if is_obj_recognition:
        ...
        object_response = generate_description_of_image(object_instruction, image)
    ...
    ########## Spatial Arrangement ##########
    instruction = build_vlm_instruction_multi_objs(prompt_meta)
    spatial_response = generate_description_of_image(instruction, image)
